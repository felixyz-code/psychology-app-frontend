...
describe('ReportsRunnerService', () => {
  ...
  let financialTransactionsService: {
  ...
  beforeEach(() => {
    ...
    caseFilesService = {
      getCaseFileByPatientId: vi.fn(() => of(createCaseFile())),
      getWorkspace: vi.fn(() => of(createWorkspace())),
    };
    ...
    patientsService = {
      getPatientById: vi.fn(() => of(createPatient())),
  ...
  it('returns an empty agenda report when no appointment matches the filters', async () => {
    appointmentsService.getAppointments.mockReturnValue(
      of([createAppointment('cancelled', '2026-07-01T11:00:00.000Z', 'CANCELLED')]),
    );
  ...
  it('builds a clinical summary from completed appointments and sorted clinical activity', async () => {
    ...
    workspace.sessionNotes = [
      createNote('older', '2026-07-01T10:00:00.000Z'),
      createNote('newer', '2026-07-02T10:00:00.000Z'),
    ];
    caseFilesService.getWorkspace.mockReturnValue(of(workspace));

    const result = await firstValueFrom(
      createService().runClinicalSummaryReport({
        patientId: 'patient-1',
        from: '2026-07-01',
        to: '2026-07-02',
      }),
    ...
    if (content?.kind === 'summary') {
      expect(content.kpis.find((metric) => metric.label === 'Sesiones completadas')?.value).toBe(
        '1 cita',
      );
  ...
  it('treats a missing case file as a valid empty clinical summary', async () => {
    ...
    const result = await firstValueFrom(
      createService().runClinicalSummaryReport({ patientId: 'patient-1' }),
    );
  ...
  it('propagates an unexpected case-file error', async () => {
    caseFilesService.getCaseFileByPatientId.mockReturnValue(
      throwError(() => ({ status: 500, message: 'Unavailable' })),
    );

    await expect(
      firstValueFrom(createService().runClinicalSummaryReport({ patientId: 'patient-1' })),
  ...
  it('builds the clinical record with documents, references, full appointments, and timeline', async () => {
    const workspace = createWorkspace();
    workspace.appointments = [
      createAppointment('scheduled', '2026-07-02T10:00:00.000Z', 'SCHEDULED'),
    ];
    workspace.documents = [createDocument()];
    caseFilesService.getWorkspace.mockReturnValue(of(workspace));

    const result = await firstValueFrom(
      createService().runClinicalRecordReport({ patientId: 'patient-1' }),
    );
    const content = result.clinicalContent;
  ...
  it('propagates an unexpected case-file error for the clinical record', async () => {
    ...
    await expect(
      firstValueFrom(createService().runClinicalRecordReport({ patientId: 'patient-1' })),
    ).rejects.toMatchObject({ status: 500 });
  });
